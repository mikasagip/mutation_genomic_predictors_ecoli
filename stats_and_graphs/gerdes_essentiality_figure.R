...
tab <- table(df_mut$essentiality_gerdes, df_mut$has_mutation)
...
plot_a <- ggplot(df_mut, aes(x = essentiality_gerdes, fill = has_mutation)) +
  geom_bar(position = "fill") +
  scale_fill_manual(values = c("TRUE" = "dodgerblue3", "FALSE" = "red3")) +
  labs(
    x = "GERDES Gene Essentiality",
    y = "Proportion",
  ...
  geom_text(
    data = pval_df,
    aes(x = essentiality_gerdes, y = y.position, label = label),
    inherit.aes = FALSE
  )
...
plot_b <- ggplot(df_nonzero, aes(x = essentiality_gerdes, y = mutation_density, fill = essentiality_gerdes)) +
...
wilcox.test(mutation_density ~ essentiality_gerdes, data = df_nonzero)
#also runs a wilcoxon test seperatly so results can be reported

outlier_info <- df_nonzero %>%
  group_by(essentiality_gerdes) %>%
  summarise(
...
df_outliers <- df_nonzero %>%
  left_join(outlier_info, by = "essentiality_gerdes") %>%
  mutate(is_outlier = mutation_density < lower | mutation_density > upper)
#makes new data frame for outliers

tab_outliers <- table(df_outliers$essentiality_gerdes, df_outliers$is_outlier)
...
plot_c <- ggplot(df_outliers, aes(x = essentiality_gerdes, fill = is_outlier)) +
  geom_bar(position = "fill") +
  scale_fill_manual(values = c("TRUE" = "dodgerblue3", "FALSE" = "red3")) +
  labs(
    x = "GERDES Gene Essentiality",
...
ggsave(
  filename = paste0(output_dir, "/essentiality_gerdes.png"),
  plot     = composite,
  width    = 10,
  height   = 5,
  dpi      = 300,
