...
tab <- table(df_mut$asymmetry, df_mut$has_mutation)
#makes them into a table

chisq_res <- chisq.test(tab, correct = FALSE)
chisq_res$p.value
chisq_res
#does a chisqr test on between the Lead and Lag conditions 
#prints the result

pval_df <- data.frame(
  asymmetry = c("leading", "lagging"),         
  y.position = 1.05,                        
...
plot_a <- ggplot(df_mut, aes(x = asymmetry, fill = has_mutation)) +
  ...
  geom_text(
    data = pval_df,
    aes(x = asymmetry, y = y.position, label = label),
    inherit.aes = FALSE
  )
...
plot_b <- ggplot(df_nonzero, aes(x = asymmetry, y = mutation_density, fill = asymmetry)) +
  geom_violin(trim = FALSE, alpha = 0.7) +
  scale_fill_manual(values = c("leading" = "darkorchid", "lagging" = "gold")) +
  stat_compare_means(
    method = "wilcox.test",
    label = "p.signif",
    comparisons = list(c("leading", "lagging")),
...
#creates a violin plot based on the asymmetry data only non-zero values
#does a wilcoxon test and labels the graph with stars or ns depending on result


wilcox.test(mutation_density ~ asymmetry, data = df_nonzero)
...
df_outliers <- df_nonzero %>%
  left_join(outlier_info, by = "asymmetry") %>%
  mutate(is_outlier = mutation_density < lower | mutation_density > upper)
#makes new data frame for outliers

tab_outliers <- table(df_outliers$asymmetry, df_outliers$is_outlier)
...
plot_c <- ggplot(df_outliers, aes(x = asymmetry, fill = is_outlier)) +
  geom_bar(position = "fill") +
  scale_fill_manual(values = c("TRUE" = "dodgerblue3", "FALSE" = "red3")) +
  labs(
    x = "Asymmetry",
...
#saves the composite figure
ggsave(
  filename = paste0(output_dir, "/asymmetry.png"),
  plot     = composite,
  width    = 10,
  height   = 5,
  dpi      = 300,
